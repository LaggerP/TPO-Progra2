...
public class Entradas {
	...
	public void CargarDMArch(DiccionarioMultipleStringTDA destino,
			DiccionarioMultipleStringTDA destinoPorcentajeEstaciones,
			DiccionarioMultipleStringTDA destinoEstacionesTranferencia) {
		...
		for (int i = 0; i < archivos.length; i++) {
			try {
				...
				while ((linea = buffer.readLine()) != null) {
					...
					// metodo split separa el string cada vez que se encuentra el delimitador. En
					// este caso ';'
					// aunque este metodo al encontrarse con string vacios los elimina. Para evitar
					// esto,
					// se coloca el -1 como limitador
					nuevo = new Info();
					clave = archivos[i].substring(0, archivos[i].length() - 4); // retorna el nombre del archivo sin la
																				// extension .csv
